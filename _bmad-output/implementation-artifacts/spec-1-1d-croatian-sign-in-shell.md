...
**Applied (2026-09-04, one review pass, four adversarial layers).**

- [x] [Review][Patch] `/prijava`'s route is never verified to render `SignInScreen` — a `component` swap or removal ships silently, the same bug shape this story caught and fixed for `notFoundComponent`. Fixed: `SignInScreen` exported, identity-asserted in `router.test.ts`, mutation-proofed (`component: () => null` fails it) [apps/web/src/router.test.ts]
- [x] [Review][Patch] `main.tsx`'s boot-gate test only checks source-text ordering (`gate < mount`), not that the mount sits inside the `if` — discarding the boolean and mounting unconditionally still passed. Fixed: added a regex assertion requiring `createRoot(` to be the immediate next token after the `if` opens, mutation-proofed [test/localization-applied.test.ts:225]
- [x] [Review][Patch] Boot-fallback reveal is verified only by a substring match for `opacity: 0` — deleting the reveal `animation`/`@keyframes` left the fallback permanently invisible on a real failure, with the suite green. Fixed: added assertions for the `animation:` declaration and that its `@keyframes` rule reaches `opacity: 1`, mutation-proofed [test/localization-applied.test.ts:401]
- [x] [Review][Patch] `#boot-fallback` is hidden via `opacity: 0` alone, which does not remove it from the accessibility tree — a screen reader can announce the Croatian boot-failure message during a normal, successful load. Fixed: added `visibility: hidden` alongside `opacity: 0`, flipped to `visible` in the same keyframe as `opacity: 1`, and asserted both [apps/web/index.html:22]
- [x] [Review][Patch] The new `forced-colors` focus-outline rule (added this iteration to fix an invisible focus ring) had no test anywhere — deleting it would silently regress the fix. Fixed: added a source-level assertion in `theme-tokens.test.ts`, mutation-proofed [apps/web/src/index.css:295]
- [x] [Review][Patch] Four `prijava.test.ts` detectors (`inputElements`, `buttonElements`, `attributeOf`, `labelTargets`) had no synthetic self-test, despite the file's header comment claiming every detector is self-tested. Fixed: added self-tests for all four [apps/web/src/routes/prijava.test.ts:452]
- [x] [Review][Patch] `resource-hygiene.test.ts`'s screen-key ICU-argument check only rejected the substring `'plural'` — a `select`/`selectordinal` argument would pass undetected. Fixed: widened to an ICU-argument-shape regex covering all three argument types [test/resource-hygiene.test.ts:119]
- [x] [Review][Patch] `prijava.test.ts`'s "stops the form from navigating" check was a bare substring search for `preventDefault()` anywhere in the file, not that it's bound to the form's own `onSubmit`. Fixed: added an assertion scoped to the `<form>...</form>` block requiring `preventDefault()` inside its `onSubmit` handler [apps/web/src/routes/prijava.test.ts:429]
- [x] [Review][Patch] `AUTHORED_VOCABULARY`'s occurrence sweep matched substrings without word boundaries — safe by coincidence, not by construction. Fixed: added `wordOccurrences()` (Unicode-letter-aware boundaries, correct for Croatian diacritics) and switched the sweep to it, with a self-test proving the gap it closes [test/localization-applied.test.ts:183]

**Investigated, no code change.**

- [x] [Review][Defer] `SANCTIONED_SCREEN_KEYS` (`resource-hygiene.test.ts`) and `prijava.test.ts`'s own key derivation look like two hand-maintained lists of the same seven keys, but they aren't: `prijava.test.ts` derives its set structurally ("every resource key that isn't a plural"), and `SANCTIONED_SCREEN_KEYS` is a deliberate, comment-documented allowlist ("nothing else in the tree may add a key without editing this list, which is the point") — an intentional review gate, not accidental duplication. No safe unambiguous fix exists that wouldn't undermine that stated intent — deferred as-is [test/resource-hygiene.test.ts:42]
- [x] [Review][Defer] The L2 branch selectors require the literal's branch to be a direct child of the expression container — a branch nested inside an intervening call (e.g. `.map(item => item.active ? 'Da' : 'Ne')`) bypasses the guard. **Attempted and reverted**: widening the branch selector to a descendant combinator closes the `.map()` gap but also matches `{t(cond ? 'a' : 'b')}` — the computed-key exemption this file's own comments explicitly protect (confirmed by a live test failure). No selector achievable in a syntax-only, non-type-aware ESLint config (this repo's Babel-parser constraint) can distinguish "a branch reached through a rendering callback" from "a branch reached through a `t()` argument." Real gap, not exploited today (no lists render on this screen), reachable by the first list-rendering story (1.5, 1.7, 1.8, or a 3.x calendar view) — worth a fresh look then, possibly with a dedicated selector scoped to JSX-returning callbacks specifically [eslint.config.js:169]
