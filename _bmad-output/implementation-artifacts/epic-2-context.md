...
- **Rotation storage rules (settled in 2.3a):** patterns and steps are immutable; steps can be inserted only while the pattern has no assignment. A pattern change (2.6) creates a new pattern plus a new assignment version pointing at it, so past dates still project through the old steps and no step versioning is needed. A new assignment version must change the pattern, offset step or anchor. The anchor lives on each assignment; a "shared" anchor is a UI convention. Saving is deliberately non-atomic (pattern, then steps in one bulk insert, then assignments in one bulk insert); an unreferenced pattern projects nothing, and a retry must build a fresh pattern, never reuse a half-written one.
- **Open decisions for 2.3b/2.6:** whether archiving a team with an in-force or scheduled assignment is refused or closes the assignment, and whether archiving a shift type used by a pattern in force is refused or only labelled (both in `deferred-work.md`).
...
- **Visual register (refreshed):** slate surfaces, a navy sidebar in both themes, one blue `primary`, soft `shadow-sh` on cards, 12 px base radius, DM Sans body and Syne headings. Screens compose the `components/ui` primitives (Button, Card, Input, Table, PageHeader/PageTitle, StatCard, Badge, Notice, Dialog and the refresh-C additions below) and may size or place them but never restyle colour, radius, border, shadow or type. A new look means changing or adding a primitive.
- **Dialogs (design refresh C):** configuration lists add in a `Dialog` opened from the list and edit in a `Dialog` over it, driven by the existing `$id` route (a link opens the record, Back closes it). Remove or archive sits beside Save, and its confirmation replaces the form inside the dialog. Standalone confirmations use `ConfirmDialog`. Also available: `Callout`, `IconTile`, `StatTile`, `InputGroup`, `OutputField`, `Timeline`, `PageDescription`, and a `dashed` Button variant. Every screen gets a lede, and fields get leading icons. Remove and archive stay neutral, never `destructive`.
...
- **From Epic 1:** teams of any count (archived, never deleted); the organization timezone, now set only at provisioning and read-only to the admin (every date still resolves in it); the RLS and role pattern with its security regression tests; the versioning mechanism; attribution defaults; and the theme and i18n layers.
- **Within the epic:** 2.1 (bands, both fixtures seeded, `packages/domain` started) and 2.2 (shift types with versioned times, editor under Postavke rotacije) are done. 2.2 came before 2.3 because steps reference shift types. 2.3 is split: 2.3a (schema `0016_rotation.sql`, `domain/projection`, teams and rotation seeded for both fixtures and the demo, anchor and effective date 2020-01-01) is done; 2.3b (builder, offsets, preview under `/postavke-rotacije`, UI-only on top of 2.3a) is next and closes the 2.3 parent. 2.3 builds the projection and cycle preview, and 2.4 (phone layout), 2.5 (warnings at save) and 2.6 (versioned change) all depend on it.
